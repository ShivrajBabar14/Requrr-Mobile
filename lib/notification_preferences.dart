  ...
  Future<void> _fetchPreferences() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          remind30DaysBefore = (data['remind_30_days_before'] is int) ? (data['remind_30_days_before'] == 1) : (data['remind_30_days_before'] ?? false);
          remind15DaysBefore = (data['remind_15_days_before'] is int) ? (data['remind_15_days_before'] == 1) : (data['remind_15_days_before'] ?? false);
          remind7DaysBefore = (data['remind_7_days_before'] is int) ? (data['remind_7_days_before'] == 1) : (data['remind_7_days_before'] ?? false);
          remindOverdue = (data['remind_overdue'] is int) ? (data['remind_overdue'] == 1) : (data['remind_overdue'] ?? false);
          emailNotifications = (data['email_notifications'] is int) ? (data['email_notifications'] == 1) : (data['email_notifications'] ?? false);
          dashboardNotifications = (data['dashboard_notifications'] is int) ? (data['dashboard_notifications'] == 1) : (data['dashboard_notifications'] ?? false);
          paymentReceivedNotifications = (data['payment_received_notifications'] is int) ? (data['payment_received_notifications'] == 1) : (data['payment_received_notifications'] ?? false);
          loading = false;
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to fetch preferences: ${response.statusCode}')),
        );
        setState(() {
          loading = false;
        });
  ...
  Future<void> _savePreferences() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final data = json.decode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to save preferences: ${data['error'] ?? 'Unknown error'}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: Padding(
                  ...
                  children: [
                    ...
                    _buildSwitchTile(
                      ...
                      (val) => setState(() => paymentReceivedNotifications = val),
                    ),
                  ],
                ),
              ),
